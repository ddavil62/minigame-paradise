...
런처는 정적 카드 그리드가 아닌 **2단계 로비 흐름**으로 동작한다.
인스턴스 1개에 최대 **2명까지** 입장 가능하며, 먼저 접속한 사용자가 호스트가 된다.

```
접속 → 로비(WS, 1/2 또는 2/2) → 스타트 → 종목 선택 → 게임 페이지로 동시 이동
```

### 1단계 — 로비

- 접속 즉시 "미니게임 천국" 타이틀과 함께 큼지막한 **플레이어 카운트**(1/2 또는 2/2)가 표시된다.
- 본인 역할이 "호스트" 또는 "게스트"로 표시된다 (먼저 접속한 사용자 = 호스트).
- WebSocket으로 실시간 동기화 — 친구가 입장하면 양쪽 화면이 즉시 2/2로 갱신된다.
- **스타트 버튼은 호스트만 누를 수 있다.** 게스트에게는 "호스트가 시작하길 기다리는 중..." 안내가 표시된다.
- 3번째 접속자는 "현재 게임이 진행 중입니다" 안내 후 WS가 즉시 종료된다.

### 2단계 — 모드 자동 분기 + 종목 선택

스타트를 누른 시점의 플레이어 수에 따라 모드가 **자동으로** 결정된다 (별도 모드 선택 UI 없음).

| 시작 시 인원 | 모드 | 동작 |
|-------------|------|------|
| 1/2 | **AI 대전** | 호스트 + 봇 1명. 종목 선택 시 런처가 해당 게임의 `bot.js`를 자동으로 child_process spawn |
| 2/2 | **인간 대전** | 호스트 + 게스트. 봇 기동 없음 |

종목 선택 화면 상단에 현재 모드가 배지로 표시되고, 카드 그리드가 나타난다.

- **종목 선택은 호스트만 가능하다.** 게스트 화면은 카드가 흐릿하게 표시되고 "호스트가 선택 중..." 오버레이가 덮인다.
...
- 게스트가 입장하면 호스트 화면이 "2/2"로 갱신되고, 호스트가 스타트 → 종목을 선택하면 양쪽이 동시에 게임 페이지(같은 도메인의 `/{gameId}/`)로 이동한다.
...
- 런처 WS 프로토콜: `LOBBY_STATE` / `PHASE` / `REDIRECT(path)` / `FULL` / `RESET` (S→C), `START` / `PICK_GAME` (C→S).
