...
| B3 쓸 연출 정상화 (2026-06-16) | 서버 쓸 룰(stealPi)은 정상(G-40 입증). 증상은 B1과 동일 통합 STATE 연출 누락 → **B1 수정으로 강탈 피 fly(origin='opp-captured') + "N월 쓸!" 토스트 정상화. 서버 무수정.** *(단, `playCardSteps` 경로에 한한 수정이었고, `chooseFloorSteps` 경로의 `applySseulAfterFullFloorClear` 호출 누락은 2026-07-30 #63에서 발견·수정됨 — 후술)* |
| R5/R6 choice 손패 fly + 순서 (2026-06-17) | B1이 `choiceFloorSrcCardId`로 손패 srcCard를 더미 fly에서 **제외만** 하고 손 fly를 미등록 → 순간이동(R5) + fly 순서 어긋남(R6) 잔존. `client.js renderState`에서 `s.choiceFloorSrcCardId`를 `_choiceSrcFlyId`로 수집 → **renderMyHand 이후** `startFlyFromHand` 등록(원본 DOM 존재 필요) + `flyTargetIds` 추가. `startFlyFromHand`를 `startFlyFromDeck`보다 **먼저** 등록해 HAND_THROW→DECK 시퀀스 순서 자연 정합(R6). 결과: 내가 낸 손패가 손에서 captured로 정상 fly(부딪힘 연출). 회귀 게이트 e2e **E-31**. *(R5 구현 시 `_choiceSrcFlyId` 발사 블록에 me/opp 분기가 없어, 상대 차례에도 내 손에서 fly가 출발하는 신규 결함이 도입됨 — 2026-07-30 #64에서 발견·수정됨)* |
...
| 버그 A 바닥 우측 슬롯 fly 착지 좌표 (2026-06-17) | fly 클론이 바닥 슬롯 카드 DOM을 `cloneNode(true)`로 복사할 때 인라인 `transform: translate(-50%,-50%) [rotate(Ndeg)]`가 함께 복사돼 도착 좌표가 **좌상단으로 어긋나던 버그**(직전 B2가 못 잡은 우측·스택 슬롯). `client.js` fly 생성 5개 함수(6개 클론 지점: `startFlyFromHand` 통상·폴백/`startFlyFromOppHand`/`startFlyFromOppCaptured`/`startFlyFromDeck`/`spawnCardClone`)에서 클론 직후 `clone.style.transform = ''` 초기화(통상 경로는 `transition:'none'` 앞에 배치). DECK_FLIP의 `rotateY(-180deg)` 별도 설정과 무충돌. 서버·룰·점수 무수정. 정량: 미수정 시 좌 ~30px/상 ~42.5px 어긋남 → 수정 후 오차 **0.02px**(`computed transform=none`). 시각 검증 `tests/fly-right-slot-visual.spec.js`(VIS-A1/A2/A3). AD3 APPROVED. |
...
| §8 R5/R8 fly·표시 | E-31~E-32 | (2026-06-17) E-31: 바닥 2장 선택 흐름에서 내 손패 srcCard fly가 myCards에서 출발(origin='hand', startFlyFromDeck 미호출, R5) / E-32: 조커를 내면 fly가 myCards에서 출발하고 조커가 `#my-captured-zone`에 안착(fade 없음, R8). **(2026-06-20 강화) E-32에 origin='hand' 조커 fly가 정확히 1개(중복 재생 금지) 단언 추가** |
