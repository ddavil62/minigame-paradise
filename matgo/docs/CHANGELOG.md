...
- `game.js`가 한 턴의 결과를 `turnAction`으로 제공한다.
  - `PLAY_MATCH`: 손패와 바닥 패 결속
  - `STAGE_DRAWN_JOKER`: 더미 조커 공개와 바닥 스테이징
  - `DRAW_MATCH`: 조커 뒤 추가 더미 결속
  - `SETTLE_CAPTURE_BATCH`: 일반 획득, 조커, 강탈 피의 최종 정산
- 턴마다 `turnId`, 정산마다 `batchId`, 완료 스냅샷에 `resultingStateVersion`을 기록한다.
- 리포트 33의 일반 획득 네 장, 조커 한 장, 강탈 피 한 장을 누락·중복 없이 같은 배치에 포함한다.
- 리포트 34의 따닥 네 장과 강탈 피 한 장을 같은 배치에 포함한다.
- 카드 소유권과 점수는 기존 서버 권위 배열을 유지하며 타임라인은 연출 정보로만 사용한다.

### UI

- `client.js`의 턴 fly 순서를 `손패 충돌 → 조커 공개 → 조커 바닥 스테이징 → 추가 더미 충돌 → 정산`으로 확장했다.
- 조커가 스테이징을 마치기 전에는 추가 더미 카드를 숨긴다.
- 강탈 피를 손패 이동 그룹에서 분리해 따닥 네 장보다 먼저 이동하지 않게 했다.
- 포획 카드, 조커, 강탈 피는 같은 `RESOLVE` 단계에서 이동을 시작한다.
- 액션 토스트는 이동 중 숨기고 모든 클론 정리 후 표시한다.

### 검증

- QA 독립 실행 `110/110 PASS`
  - 리포트 33·34 핵심 규칙·Chromium 반복: `8/8`
  - 기존 규칙·리포트 27~30·UI 회귀: `60/60`
  - 쓸·조커·폭탄 직접 회귀: `42/42`
- 1920×1080 Chromium에서 조커 스테이징, 추가 드로우, 여섯 장 정산과 따닥 동시 이동을 확인했다.
- Art Director 모드 3 `APPROVED`.
- 같은 3013 테스트 서버를 공유하는 Playwright 묶음을 병렬 실행하면 `/test/reset`이 충돌할 수 있다. 관련 120초 대기 1건은 파일 단독 재실행 `2/2 PASS`로 격리했으며 제품 결함이 아니다.
- 에셋 변경 없음.

### 참고

- 목적 정의: `.Codex/specs/2026-07-27-minigames-reports-33-34-scope.md`
- 실행 계획: `.Codex/specs/2026-07-27-minigames-reports-33-34-plan.md`
- 구현: `.Codex/specs/2026-07-27-minigames-reports-33-34-report.md`
- UI 검수: `.Codex/specs/2026-07-27-minigames-reports-33-34-ui-review.md`
- QA: `.Codex/specs/2026-07-27-minigames-reports-33-34-qa.md`
