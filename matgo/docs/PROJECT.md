...
공개 HTTPS 통합 런처에 접속한 두 PC가 즐기는 2인 고스톱(맞고). 친구는 같은 공유기에서도 `https://112.155.2.238`을 사용하며 개별 포트 직접 접속은 운영하지 않는다. Node.js + WebSocket 기반의 권위적 서버(룰 100% 서버 판정), 클라이언트는 렌더링·입력 송신만 담당. 한국 고스톱 담요 톤의 UI(2026-05-28부터 v8 시안 적용).
...
| 기능 | 설명 | 상태 |
|------|------|------|
| 공개 HTTPS 1:1 매칭 | 두 PC가 같은 공개 서버에 접속 시 자동 방 매칭 | 완료 |
