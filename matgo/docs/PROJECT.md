...
| 클라이언트 | `public/client.js` | STATE 렌더·입력 송신, 소유권 기반 fly, 단일 batch 중복 방지와 최종 fly 뒤 효과 토스트 |
...
| 폭탄 후 추가 2장 뒤집기 | 손패가 없어도 더미 입력 1회로 권리를 사용하고 선택 대기에서도 진행 주체·남은 횟수 보존 | 완료 |
...
| fly 애니메이션 | 절대 출처·소유권으로 손·더미·강탈 이동을 재생하고 batch/card별 1회 등록, 최종 clone 제거 뒤 효과 표시 | 완료 |
| 원자 포획 정산 | 선택·후속 더미 전 획득 예정 카드를 보류하고 #39 네 장·#40 여섯 장을 단일 batch로 확정 | 완료 |
...
- **레거시 테스트 하네스**: `e2e-scenarios`, `entry-ui-qa`, `floor-joker-smoke` 일부는 현재 닉네임·자동 READY 게이트를 반영하지 못한다. 현행 수용 경로는 unit/rules/log/runner `117/117`, 직접 규칙 `42/42`, 신규 browser+timing 3회 `33/33`, #33~#34 browser 3회 `6/6`, 효과 timing 3회 `15/15` PASS.
