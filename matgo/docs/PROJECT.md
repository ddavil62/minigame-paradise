...
| 폭탄 후 추가 2장 뒤집기 | 통상 뒤집기 뒤 같은 턴에 두 장을 추가 해결하며 선택 대기에서도 진행 주체·남은 횟수 보존 | 완료 |
...
| fly 애니메이션 | 턴 액션 순서에 따라 손패·조커·추가 더미를 직렬 재생하고 포획 카드·강탈 피를 한 배치로 정산 | 완료 |
...
- **레거시 테스트 하네스**: `e2e-scenarios`, `entry-ui-qa`, `floor-joker-smoke` 일부가 현재 닉네임·자동 READY 게이트를 반영하지 못한다. `report-ui-regression`은 테스트 주입 직후 초기 fly 안정화 대기가 필요하다. 제품 수용기준은 독립 규칙·안정화 Chromium 경로 `151/151` PASS.
