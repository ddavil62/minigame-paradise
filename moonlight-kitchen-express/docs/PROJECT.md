...
로비에 처음 접속해 첫 스냅샷을 받은 뒤 4페이지 플레이 가이드가 한 번 자동으로 열린다. 확인 여부는 버전이 포함된 `moonlightKitchenGuideSeen:v1` 로컬 저장 키에 기록되어 새로고침해도 강제 재노출되지 않는다. 이후에는 로비와 운행 중의 `? 플레이 방법` 버튼으로 언제든 다시 열 수 있으며, 연결·결과·종료 확인 같은 상위 차단 화면과는 동시에 열리지 않는다.

| 페이지 | 안내 내용 |
|---|---|
| 주문 완성하기 | 주문 확인 → 재료를 `E`로 집기 → 작업대에서 `Space` 유지 → 빈 접시를 완성 설비에 들고 가 `E`로 담기 → 정차 중 배식대에 전달하는 5단계 |
| 세 가지 레시피 | 달버섯 꼬치, 등불잎 만두, 혜성무 국수의 재료·손질 설비·조리 설비와 별국수의 손질 생략 규칙 |
| 과열 함께 식히기 | 열도 85 이상에서 P1은 냉각 펌프, P2는 배기 밸브에서 `Space`를 동시에 유지하는 협동 규칙 |
| 조작 익히기 | WASD/방향키 이동, `E`/`Enter` 상호작용, `Space` 작업 유지, `Q` 바닥 내려놓기 |

한국어와 영어의 제목·본문·버튼·접근성 레이블을 모두 제공한다. 가이드가 열리면 눌린 키를 즉시 초기화하고 이동·상호작용·작업·버리기 입력을 차단하며, 닫힌 뒤 새 입력부터 정상 처리한다.

dialog 의미 구조, Tab/Shift+Tab 포커스 트랩, Escape 닫기, 닫은 뒤 로비 opener 또는 운행 Canvas로의 초점 복원, `aria-live` 페이지 변경 알림, 44px 조작 대상, reduced-motion을 지원한다. 1280×720, 1024×768, 390×844에서 dialog와 기존 HUD가 겹치지 않도록 검증했다. 2026-07-19 최종 회귀 결과는 Node/WebSocket 31/31, Playwright 15/15 PASS다.
