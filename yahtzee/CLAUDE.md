...
└── tests/
    ├── smoke.test.js        # YACHT-001~012 + LIVE-001/002 (169건)
    ├── dice-render.test.js  # YACHT-LIVE-003/004 + YACHT-KEEP-001~008 (55건, DOM stub 노드 러너)
...
node tests/smoke.test.js        # 169건 (YACHT-001~012 + LIVE-001/002)
node tests/dice-render.test.js  #  55건 (YACHT-LIVE-003/004 + YACHT-KEEP-001~008)
...
기대: `총 N건, PASS=N, FAIL=0`. 합계 **249 / 249 PASS** (smoke 169 + dice-render 55 + bot-smoke 25).
...
| **컵 진행 중 미리보기 게이트 (N3)** | 컵 굴림 애니메이션 중에는 카테고리 점수 미리보기를 숨긴다. `main.js renderAll`이 `canPreview = !els.diceArea._cupTimer`를 계산해 `renderScoreboard(ctx)`에 전달, `scoreboard.js`가 `ctx.canPreview !== false` 게이트로 `canSelectCategory`를 한정(undefined는 true — 구 호출부 호환). 컵 착지 시 `dice.js`가 `opts.onCupDone()` 호출 → `main.js`의 `onCupDone: () => renderAll()`이 재렌더해 미리보기 자동 노출. **무한 루프 가드**: 재렌더 시 rollCount가 불변이고 `rolledNow=true` 진입 즉시 `_lastRollCount`가 갱신됐으므로 `rolledNow=false` → 컵 분기 미진입 → onCupDone 재등록 없음. (2026-06-20 트리거 교체 전엔 diceChanged=false 가드였음.) |
| **컵 굴림 트리거는 값 변화가 아닌 rollCount 증가 (2026-06-20)** | `dice.js`의 컵 흔들기/드롭 발동은 dice 값이 바뀌었는지가 아니라 서버 권위 `rollCount` 증가(`opts.rollCount > container._lastRollCount`)로 판정한다. keep 안 한 다이스가 우연히 직전과 **전부 같은 면**으로 굴려져도(1/6~1/36) 정상 발동해야 한다. `main.js`가 `renderDice` opts에 `rollCount: state.rollCount`를 반드시 전달해야 하며(누락 시 `diceChanged` 폴백으로 회귀), `rolledNow=true` 진입 즉시 `_lastRollCount`를 기록해야 무한 루프가 안 난다. 턴 넘김으로 rollCount가 줄면(리셋) else 분기에서 `_lastRollCount`도 낮춰야 새 턴 첫 굴림(0→1)이 발동한다. 회귀: YACHT-KEEP-006(직격)/007/008, smoke YACHT-012. |
