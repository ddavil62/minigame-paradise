 ...
 * @param {{hostUrl?:string,testing?:boolean,now?:()=>number,seed?:number,duration?:number,disconnectGraceMs?:number}} [options] 설정
...
export function createApp(options = {}) {
  ...
  function broadcastRoom() {
    for (const slot of room.slots.filter((entry) => entry.connected)) send(slot.ws, { type: 'ROOM_STATE', myId: slot.id, players: room.slots.map((entry) => ({ id: entry.id, name: entry.name, ready: entry.ready, connected: entry.connected })) });
  }
  ...
  function maybeStart() {
    ...
    active.forEach((slot) => room.game.addPlayer(slot.id, slot.name)); room.game.start();
  ...
  function handleMessage(slot, message) {
    ...
    if (message.type === 'RETURN_LOBBY') { slot.ws.close(); return; }
    ...
    if (message.type === 'MATCH_PAIR') {
      ...
      send(slot.ws, { type: result.ok ? 'PAIR_ACCEPTED' : 'PAIR_REJECTED', ...result });
  ...
  wss.on('connection', (ws, request) => {
    const url = new URL(request.url || '/', 'http://localhost'); const token = url.searchParams.get('sessionToken');
    ...
    if (slot) { slot.ws = ws; slot.connected = true; clearTimeout(slot.disconnectTimer); }
    else {
      const id = room.slots.some((entry) => entry.id === 'p1') ? 'p2' : 'p1'; slot = { id, token: crypto.randomUUID(), name: normalizeName(url.searchParams.get('name')), ready: false, ws, connected: true, graceUntil: 0, disconnectTimer: null, rateEvents: [], rateViolations: 0, oversizeEvents: [] }; room.slots.push(slot);
    }
    send(ws, { type: 'JOINED', playerId: slot.id, sessionToken: slot.token, hostUrl, reconnected: Boolean(token), waiting: !room.game });
    ...
    ws.on('close', () => {
      ...
      slot.disconnectTimer = setTimeout(() => { if (!slot.connected && room.game && !room.game.result) { const winner = room.slots.find((entry) => entry !== slot && entry.connected); room.game.finish(winner?.id || null, 'disconnect'); syncAll(); } }, disconnectGraceMs + 50);
    });
  });
  ...
  return { handleHttp, handleUpgrade, setHostUrl(value) { hostUrl = value; }, close() { clearInterval(room.tick); room.slots.forEach((slot) => clearTimeout(slot.disconnectTimer)); wss.close(); } };
}

if (process.argv[1] === fileURLToPath(import.meta.url)) {
  const app = createApp({ testing: process.env.SICHUAN_E2E === '1', seed: process.env.SICHUAN_E2E === '1' ? 4242 : undefined }); const port = Number(process.env.PORT || 3018); const server = http.createServer(app.handleHttp);
