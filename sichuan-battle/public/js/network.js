...
export class GameNetwork extends EventTarget {
  ...
  constructor(name){super();this.name=name;this.socket=null;this.sequence=0;this.closed=false;this.reconnectTimer=null;this.socketGeneration=0;this.connect();}
  ...
  connect(){
    ...
    const socket=new WebSocket(`${protocol}://${location.host}/sichuan-battle/ws?name=${encodeURIComponent(this.name)}&sessionToken=${encodeURIComponent(token)}`);this.socket=socket;
