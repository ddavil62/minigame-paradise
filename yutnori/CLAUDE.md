...
- **2026-06-17 버그 B/C/D 수정 완료**: B — 준비 버튼 :hover 위치 튐(CSS, `.ready-btn:hover:not(:disabled)`에 `transform: translate(-50%,-50%) scale(1.04)` 추가로 `.primary-btn:hover` translateY override) / C — 대기 화면 초대 주소 패널 제거 → 요트다이스 P1/P2 ready 패턴 통일(서버 hostUrl/setHostUrl·JOINED 필드는 유지, 클라 표시만 제거) / D — 윷·모로 말 이동 시 보너스 던지기 소실(서버, MOVE_PIECE/CHOOSE_PATH에서 `bonusFromConsumed = useResult==='yut'||'mo'`를 splice **이전**에 계산해 hasBonus에 포함). 회귀 YR-C8-009 단언을 해소 룰 기댓값(`currentTurn='p1'`+`pendingResults=[]`)으로 갱신, e2e E-02 invite-panel→`#ready-status` 갱신. 서버리스 338 + bot-smoke 10 + e2e 25 전부 PASS, AD3 APPROVED.
...
| 윷·모 보너스 splice 순서 (2026-06-17 버그 D) | MOVE_PIECE/CHOOSE_PATH는 소비한 결과(`useResult`)를 `pendingResults.splice`로 큐에서 먼저 제거한 뒤 `lastResult`를 읽는다. 윷·모를 소비하면 큐가 비어 `lastResult=null`·`hasBonus=false`가 되어 `passTurn()`이 오호출, 보너스 던지기가 소실된다. **반드시 splice 이전에 `bonusFromConsumed = useResult==='yut'||useResult==='mo'`를 계산**하고 `hasBonus = capturedBonus || bonusFromConsumed || lastResult==='yut'||'mo'`로 확장한다. 두 핸들러 동시 적용. capturedBonus 소진/리셋은 §13-12 가드 그대로(윷·모 잡기 중복 미부여). |
