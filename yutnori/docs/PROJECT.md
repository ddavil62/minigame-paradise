...
> 공개 HTTPS 통합 런처에서 즐기는 한국 전통 윷놀이 대전. 사람 2~4인과 사람 1명 + AI 1명의 1:1 대전을 지원한다. 친구는 같은 공유기에서도 `https://112.155.2.238`로 접속하며 포트 3000 직접 접속은 허용하지 않는다.

## 현재 상태 (2026-07-27)

**리포트 31 — 모서리 말 판정·표시 통일** — 경로상의 `cell`과 실제 보드 위치를 분리하는 정규 노드 ID를 도입했다. 서버의 잡기·업기·묶음 이동과 클라이언트의 그룹화·클릭·렌더링이 동일한 노드 기준을 사용하며, 레거시 시작점 `cell 20`도 현재 시작점 `cell 0`과 같은 `node:0`으로 처리한다. 보드 위 모든 말은 플레이어와 칸 종류에 관계없이 같은 크기로 노드 중앙에 표시한다. AD3 APPROVED, QA **66/66 PASS**.
...
- ✅ 말 4×2색 렌더링 + 업힘 카운트 표시 + 정규 노드 중앙 정렬·동일 크기
...
- 같은 정규 노드의 자기 말 묶음을 "업힘 1묶음"으로 시각화하고, 이동 시 같은 node ID의 자기 piece 인덱스를 함께 이동시킴 (의도된 단순화)
