...
## [2026-07-27] — 리포트 31 모서리 말 판정·표시 통일

### 추가

- `public/js/board-node.js`: 경로상의 `cell`을 실제 보드 위치인 정규 노드 ID로 변환하는 공통 모듈을 추가했다. HOME과 GOAL은 충돌 대상에서 제외하고, 레거시 시작점 `cell 20`과 현재 시작점 `cell 0`은 모두 `node:0`으로 처리한다.
- `tests/report31-regression.spec.js`: HOME에서 모로 모서리 도착, 정규 노드 별칭, 고정 말 렌더링, 실제 WebSocket 잡기 회귀 4건을 추가했다.
- `tests/report31-browser.spec.js`: 1920×1080 실제 캔버스에서 모서리 잡기 전후의 중앙 정렬과 HOME 복귀를 검증하는 브라우저 테스트를 추가했다.

### 변경

- `server.js`: 잡기·업기·이동 전 묶음 수집을 정규 노드 기준으로 통일하고, STATE의 각 말에 후방 호환 가능한 `nodeId`를 제공한다.
- `public/js/piece.js`: STATE에 `nodeId`가 있으면 이를 사용하고, 구버전 상태는 `cell`에서 다시 계산해 같은 물리 노드의 말을 선택한다.
- `public/js/ui.js`: 말 그룹화를 정규 노드 기준으로 변경했다. 보드 위 모든 말은 플레이어·일반 칸·모서리 구분 없이 반지름 15로 렌더링하며, P2에 적용되던 `(+6, -4)` 오프셋을 제거해 노드 중앙에 표시한다.

### 수정

- 모가 나온 말이 모서리 지름길 진입점에 도착했을 때 플레이어에 따라 크기와 위치가 달라 보이던 문제를 수정했다.
- 같은 물리 노드를 서로 다른 경로 `cell`로 표현한 상태에서도 상대 잡기, 같은 편 업기와 묶음 이동이 일관되게 동작하도록 수정했다.

### 검증

- QA **66/66 PASS**: Playwright 47/47, AI smoke 10/10, 정규 노드 별칭 독립 프로브 3/3, 모바일 Chromium 게이트 6/6.
- 사람 2·3·4인 턴·잡기·승리·재대전, AI 3연전, 지름길·백도·잡기 보너스 회귀를 통과했다.
- Chromium 1920×1080 및 390×844에서 말 크기·중앙 정렬·업기 숫자·클릭·잡기 전후를 확인했고 콘솔·페이지 오류는 없었다.
- AD 모드 3 **APPROVED**. 기존 모바일 긴 상태 문구의 헤더 줄바꿈은 보드와 조작 영역을 침범하지 않는 비차단 관찰로 남겼다.

### 운영 참고

- 이미지 에셋 변경 없음. Mockup Sync 생략.

### 참고

- 구현 리포트: `.Codex/specs/2026-07-27-minigames-reports-yut-report.md`
- UI 검수: `.Codex/specs/2026-07-27-minigames-reports-yut-ui-review.md`
- QA: `.Codex/specs/2026-07-27-minigames-reports-yut-qa.md`
