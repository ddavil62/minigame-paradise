...
## [2026-08-13] - 접속 안내를 공인 IP HTTPS로 통일

- 루트 개발 지침, 운영 가이드, 메인 README와 게임별 README/PROJECT/CLAUDE 문서의 친구 접속 안내를 `https://112.155.2.238`로 통일했다.
- 같은 공유기에서도 공인 HTTPS 주소를 사용하고, 호스트 IP의 포트 3000이나 개별 게임 포트를 친구에게 안내하지 않도록 명시했다.
- 포트 3000은 Caddy의 `127.0.0.1:3000` 내부 업스트림과 서버 PC 개발·점검 전용이며 방화벽 인바운드 차단과 비밀번호 인증을 유지하도록 운영 원칙을 명시했다.
- 런처 화면의 "LAN 파티"와 "친구는 호스트 IP로 접속" 문구를 공개 HTTPS 초대 주소 안내로 교체했다.

## [2026-08-12] - 공인 IP HTTPS 접속·비밀번호 게이트·친구 초대 UI

- `MINIGAME_PASSWORD` 설정 시 런처와 16종 게임의 모든 HTTP·WebSocket 요청을 공통 로그인으로 보호한다. 미설정 LAN 실행은 기존 동작을 유지한다.
- 최초 접속 시 닉네임 입력 전에 비밀번호 UI를 표시한다. 기본 선택된 "30일간 기억"은 `Secure`, `HttpOnly`, `SameSite=Strict` HMAC 서명 쿠키를 발급하며, 선택 해제 시 12시간 브라우저 세션으로 동작한다.
- 동일 Host Origin WebSocket 검사, 출발지별 5회 실패/15분 차단을 추가했다.
- 런처 내부 AI 봇은 프로세스 시작마다 생성되는 256비트 내부 토큰으로만 인증을 우회하며 토큰 포함 URL을 로그에 남기지 않는다.
- Caddy가 현재 공인 IPv4로 Let's Encrypt 단기 인증서를 자동 발급·갱신하고 앱 포트 3000으로 역방향 프록시하는 `start-public.ps1/.bat`과 `infra/Caddyfile.ip`을 추가했다. 공인 인터페이스의 3000 포트는 차단하고 Caddy 실행 파일에만 80/443 인바운드를 허용한다.
- 인증된 로비에 `https://공인IP`를 표시하고 복사하는 친구 초대 패널과 보호된 `/connection-info` API를 추가했다.
- 인증·presence·채팅 단위 테스트 12건, 신뢰되는 공인 HTTPS의 로그인·30일 쿠키·초대 API·presence·1:1 채팅·WSS 통합 검사, 데스크톱/모바일 Chromium UI 검사를 통과했다.

### 현재 접속자 presence

- 인증된 모든 HTML에 공통 `presence-client.js`를 주입하고 `/presence/ws` 채널을 추가해 로비·게임 대기실·플레이 중 상태를 실시간으로 집계한다.
- 로그인 세션의 비가역 지문으로 여러 탭을 한 명으로 통합하며, 여러 탭이 열려 있으면 `플레이 중 > 대기실 > 로비` 순으로 대표 상태를 표시한다. 원본 쿠키·IP·토큰은 클라이언트에 공개하지 않는다.
- 닉네임 길이·제어문자·양방향 제어문자와 게임 ID를 서버에서 검증하고, ping/pong heartbeat로 끊긴 연결을 정리한다.
- 모든 인증 페이지 우측 하단에 현재 접속 인원 배지가 붙은 원형 친구 아이콘을 추가했다. 버그리포트 버튼 바로 왼쪽에 배치되고 기본은 최소화 상태이며, 클릭하면 Battle.net·Steam 스타일의 소형 목록에서 닉네임, 내 사용자 표시, 로비·게임 대기실·플레이 중 상태를 확인할 수 있다.
- 친구 행을 선택해 1:1 대화로 전환하는 메신저 UI, 양방향 말풍선, Enter 전송·Shift+Enter 줄바꿈, 상대 오프라인 처리, 읽지 않은 메시지 배지를 추가했다.
- 읽지 않은 메시지가 있으면 최소화 친구 아이콘이 점멸하고 짧은 2음 알림을 재생한다. 연속 알림음은 1.2초 간격으로 제한하며 모션 감소 환경에서는 애니메이션 대신 고정 테두리 강조를 사용한다.
- 채팅은 기존 인증 WebSocket에서 송신자·수신자 세션에만 전달하고, 500자 제한·제어문자 제거·10초당 20개 전송 제한을 적용한다. 최근 대화는 대화별 100건·24시간까지 서버 메모리에 보관한다.
- 접속이 끊긴 옛 세션의 대화 캐시가 누적되지 않도록 10분마다 전체 대화를 순회해 24시간이 지난 기록과 빈 대화를 제거하는 정리 타이머를 추가했다.
- 인증·presence·채팅 단위 테스트 12건, 공인 HTTPS에서 중복 세션·두 번째 사용자·상태 전환·1:1 전달·대화 기록 통합 검사, 데스크톱/모바일 Chromium 양방향 채팅 UI 검사를 통과했다.
...
- AI는 서버 사전과 두음법칙을 재사용하며 기존 `WORD_SUBMIT`·`REMATCH`만 전송하고, forced·공용 중복·게이지·막다른 체인과 stale 예약을 고려한다.
- 신규 Node 테스트 3건, 신규 런처/모바일 Playwright 2건, 기존 끝말잇기 회귀 17건을 통과했다. 신규 에셋은 없고 AD 모드 3 UI 검수가 필요하다.
...
- `launcher/public/games.json`의 사천성 카드를 AI 지원으로 전환하고, 런처 `AI 채우기`와 게임 내 CTA를 동일한 서버 권위 `REQUEST_AI` 경로로 연결했다.
- 서버 발급 일회성 토큰으로 인증한 WebSocket AI가 독립 12×8 보드에서 실제 합법 짝을 제거하고 기존 7종 아이템을 상황별로 사용한다.
- 가변 지연·숙고·후보 무작위화를 사용한 보통 난이도와 ko/en AI 표시명·텍스트 배지·상태·재대결 UI를 추가했다.
- 15초 사람 재접속 동안 AI를 일시 정지·복원하고 재대결, 명시 이탈, 유예 만료와 서버 종료에서 봇·예약·방 상태를 정리한다.

### 수정

- 런처의 사천성 AI 채우기가 무토큰 레거시 봇을 생성하지 않고 `mode=ai` 리다이렉트 뒤 게임 서버의 인증 봇 생성 경로를 사용하도록 수정했다.
- AI가 아이템·보드 revision·방해 효과 변화마다 행동 예약을 갱신하고 최신 snapshot에서 합법 짝과 아이템 사용을 재평가하도록 했다.
- AD 모드 3 피드백에 따라 재대결 토스트 잔존, 모바일 토스트 겹침, AI 명칭 중복과 영문 배지 줄바꿈을 수정했다.

### 검증

- 독립 공격 테스트 2/2, AI 정책·프로토콜·런처 통합 9/9, 전체 Node 41/41, Playwright 5/5를 통과했다.
- AI 판단 성능은 median 0.89ms, p95 1.31ms이며 AD 모드 3 재검수는 `APPROVED`다.
...
## [2026-07-23] - 사천성 배틀 실전형 무작위 보드

### 수정

- 가로 인접 동일 문양 48쌍을 사전 생성하던 구조를 제거하고, 12×8 전체 96개 위치에 24 face를 각각 4장씩 결정적으로 분산했다.
- 실제 최대 2회 꺾임·외곽 경로로 검증된 제거 순서를 함께 만드는 공용 생성기를 초기 보드와 수동·강제·자동 셔플에 적용했다.
- 셔플 과정에서 남은 face 다중집합, 타일 ID, 좌표와 제거 상태를 보존하고 서버 내부 해답은 공개 snapshot에서 제외했다.
- E2E는 단일 2인 room의 spec 간 교차 매칭을 막도록 직렬 실행하고, 효과 칩 위치 검사의 DOM 교체 레이스를 제거했다.

### 검증

- 고정 시드 10,000개에서 잘못된 타일 구성·완주 실패·과거 48개 가로 동일쌍 패턴이 모두 0건이며 배치 지문 10,000개가 모두 고유했다.
- 인접 동일 face 비율은 전체 3.4367%, 가로 3.3193%, 세로 3.5598%, 기존 가로 슬롯 3.1617%로 수용 범위를 통과했다.
- 진행도별 연속 셔플 1,800/1,800, Node 32/32, 기본 Playwright 연속 2회 4/4를 통과했다.
...
## [2026-07-23] - 사천성 배틀 추가

### 추가

- `sichuan-battle/` 신규 게임을 추가했다. 두 플레이어는 같은 시드의 12×8 초기 배치를 각자 독립 보드로 받아 3분 동안 최대 2회 꺾임과 보드 외곽 연결을 허용하는 사천성 규칙으로 경쟁한다.
- `lib/{prng,pathfinder,board,solver,items,game}.js`에 결정적 시드, 서버 경로 판정, 24종×4장 보드, 완주 가능한 셔플, 7종 아이템과 경기 상태를 구현했다. 선완주 또는 시간 종료 시 제거한 쌍 수로 승패를 확정한다.
- 성공한 첫 제거는 아이템을 확정 지급하고 이후에는 72% 드롭과 2회 연속 미획득 방지 pity를 적용한다. 3슬롯에서 1.25초 간격으로 잠금·뒤집기·강제 셔플·안개·힌트·정화·방어막을 수동 사용한다.
- `server.js`에 개인화 snapshot, 보드 revision, `matchId`·`requestId` 검증, 15초 세션 복구, 이탈 승패, 결과 후 양측 동의 재대결을 구현했다. 전체·짝 제거·아이템 사용별 rate limit과 반복 8KB 초과 연결 종료를 포함한다.
- 자동 교착은 450ms 외곽 경고 뒤 합법 수를 복구하고, 강제 셔플은 0.8초 경고 뒤 실행한다. 정화는 예약된 강제 셔플을 취소하며 셔플과 힌트 대상 제거 시 낡은 힌트를 즉시 정리한다.
- `public/`에 대기·플레이·결과 화면, ko/en 문자열, 키보드 1/2/3 아이템 사용, 저감 모션, 뒤집기 공통 뒷면, 내·상대 효과 남은 시간과 셔플 경고를 추가했다.
- GPT Image 2 기반 공용 도자 타일 몸체와 야간 사천 정원 배경을 적용했다. 24개 문양은 해·초승달·북극성부터 나침별·도자 화병까지 고유 SVG로 합성하고 256×320 PNG로 런타임에서 사용한다.
- `launcher/server.js`와 `launcher/public/games.json`에 `/sichuan-battle/`, `/sichuan-battle/ws`, 2인·AI 미지원 메타데이터를 등록했다.

### 변경

- AD2 Round 1에서 6개 추상 패턴의 반복과 무거운 SVG 구조를 발견해 24종 고정 의미·고유 실루엣의 경량 SVG로 교체했다. 48×60·56×70·64×80 컬러/그레이 축소표와 자동 에셋 검증을 추가한 뒤 Round 2 `APPROVED`를 받았다.
- AD3 Round 1에서 뒤집힌 타일 문양 노출, 강제 셔플 예고 누락, 지속 효과 시간 누락을 발견해 공통 뒷면·0.8초 비차단 경고·0.1초 단위 효과 시간을 추가했다. QA 수정 뒤 아이템 설명·ARIA, 자동 교착 경고와 재대결 대기 상태까지 Round 3에서 재검수해 `APPROVED`를 유지했다.

### 수정

- QA에서 확인한 stale 경기 요청, 경기 중 재대결, 메시지 폭주·반복 대형 메시지, 힌트 수명, 즉시 자동 셔플, 첫 재대결 표 처리 문제를 서버 권위 검증과 상태 수명 정리로 해소했다.

### 검증

- 공격적 독립 QA 11/11, 전체 단위·프로토콜 24/24, Chromium Playwright 4/4와 런처 HTTP/WS smoke를 모두 통과했다.
- 1,000개 시드의 완전 해답, 100,000회 아이템 가중치, 10,000경기 드롭 공정성, 1366×768·1024×768 ko/en 레이아웃과 콘솔 오류 0건을 검증했다.

### 참고

- 스펙: `.Codex/specs/2026-07-23-sichuan-battle.md` (`COMPLETED`)
- 구현 리포트: `.Codex/specs/2026-07-23-sichuan-battle-report.md`
- 에셋 검수: `.Codex/specs/2026-07-23-sichuan-battle-art-review.md` (`APPROVED`)
- UI 검수: `.Codex/specs/2026-07-23-sichuan-battle-ui-review.md` (`APPROVED`)
- QA: `.Codex/specs/2026-07-23-sichuan-battle-qa.md` (`PASS`)
- 에셋 변경이 있으므로 Doc Writer 뒤 `studio-mockup` 동기화와 별도 레포 커밋·푸시가 필요하다.
