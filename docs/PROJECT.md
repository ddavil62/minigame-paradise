...
> 최종 업데이트: 2026-07-19 — **별빛 우편탑 월드 확장 보완 페이즈 R.** 신규 4월드의 독립 지형·서버 권위 동적 기믹, 예상/최고 기록 UI를 완성했으며 최종 QA 46/46 PASS.
...
LAN 환경에서 2~5인이 즐기는 미니게임 12종 통합 패키지. 단일 포트(3000)에서 통합 라우터로 런처와 각 게임을 경로별로 제공한다. 게임 포탈에서 카드를 선택해 독립 대기실에 입장하고, 최소 인원 이상이 모두 준비하면 게임이 시작된다.
...
| 게임 목록 | `launcher/public/games.json` | 12종 게임 메타데이터 (경로, 봇 지원, `minPlayers`/`maxPlayers` 인원 범위) |
...
| 기능 | 설명 | 상태 |
|------|------|------|
| 단일 포트 통합 라우터 | HTTP/WS를 경로별로 12개 게임에 dispatch | 완료 |
| 게임 포탈 | 닉네임 게이트 통과 후 게임 카드 12개 표시. 카드 클릭으로 대기실 입장 | 완료 |
...
- 봇은 matgo, yutnori, yahtzee, rummikub, omok, janggi, tetris-battle, codenames에서 지원한다. hanabi, codenames-duet, davinci-code, starlight-mail-tower는 AI를 지원하지 않는다.
