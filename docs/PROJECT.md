...
> 최종 업데이트: 2026-07-30 — **맞고 버그 리포트 #54~#59 일괄 수정.** 바닥 2장 선택 시 중복 fly 제거(액션 키 가드), 뻑 후 500ms 딜레이 삽입(`ppeokDelayActive` 플래그), 상대 손 영역 하단 잘림 해소(overflow visible + min-height 176px), 따닥/쪽 토스트 즉시 표시(`stateQueue.length > 0` 무조건 차단 제거), 고 버튼 비활성화(손패 0 + 크레딧 0). 자동화 테스트 163/163 PASS. QA PASS.
