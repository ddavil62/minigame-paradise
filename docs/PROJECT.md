...
> 최종 업데이트: 2026-07-27 — **윷놀이 모바일 보드·AI 채우기 보정.** 390×844에서 보드를 374×374 정사각형으로 확보하고 하단 인포바를 게임 메인 내부 스크롤로 제공한다. 사람 대전 2~4인은 유지하되 AI 채우기는 `aiFillTargetPlayers=2`로 사람 1명과 AI 1명만 배치하며, 런처 READY를 게임 내부에서 다시 요구하지 않는다. Phase E QA 136/136 PASS.
...
| 루미큐브 | 2~4인 타일 게임 | 완료 |
| 오목 | 2인 오목 대전 | 완료 |
| 별빛 우편탑 | 협동 부스트와 장치 연동 복귀 경로를 사용하는 17개 레벨 LAN 2인 등반. CSS Grid 3행(topbar/play-viewport/bottombar) 레이아웃으로 메타 UI와 플레이 캔버스를 완전 분리. 플레이어별 안전 리스폰, AI·친구 2인 즉시 시작, 15초 재접속과 오래된 토큰 차단 지원 | 완료 |
