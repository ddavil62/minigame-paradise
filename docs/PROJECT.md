...
> 최종 업데이트: 2026-07-19 — **베네치아 조작·급류 규칙 개선.** 아이템은 입력창 포커스 중에도 단독 숫자키 1/2/3과 숫자패드 1/2/3으로 사용한다. `item_freeze` 프로토콜 ID는 유지하되 표시와 효과는 🌊 급류(4초간 상대 단어 낙하 2배)로 교체했으며, 서버 권위 누적 낙하 시계가 Canvas와 만료를 함께 결정한다. 일반 정답은 단어 제거·콤보·아이템 드랍만 처리하고 상대 HP/HIT를 만들지 않는다. 기존 HP·회복·`hp_zero` 호환 구조는 남아 있지만 현재 정상 플레이에는 HP를 0으로 만드는 경로가 없어 무한 경기가 가능하다.
...
| 베네치아 타이핑 배틀 서버 | `venezia/server.js` | 서버 권위 타이핑 대전 — 단어 생성·제출 검증, 누적 낙하 시계·급류·아이템 효과 관리 |
...
| 베네치아 타이핑 배틀 | 한메/한컴 타자교사 스타일 2인 대전 타이핑. 단어 제거·콤보 드랍과 아이템 방해 중심, 급류(4초간 2배 낙하) 포함 아이템 5종, 숫자키 1/2/3 조작, AI 봇 지원. 현재 자동 종료 규칙 없음 | 완료 |

## 알려진 제약사항

- 봇은 matgo, yutnori, yahtzee, rummikub, omok, janggi, tetris-battle, codenames, venezia에서 지원한다. hanabi, codenames-duet, davinci-code, starlight-mail-tower, moonlight-kitchen-express는 AI를 지원하지 않는다.
...
- 베네치아는 일반 정답의 HP 피해를 제거했지만 HP·회복·`hp_zero` 호환 구조를 유지한다. 정상 플레이에서 HP가 감소하지 않아 회복 아이템은 실질 효과가 없고, 기권·이탈 전에는 경기가 자동 종료되지 않을 수 있다.
- 모바일 반응형 미지원.
- LAN 전용 설계 (인증/보안 없음).
