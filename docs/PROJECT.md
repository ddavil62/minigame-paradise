...
> 최종 업데이트: 2026-06-28 — **코드네임 클래식 신규 추가(11번째 게임, 11종).** 정통 Codenames(단일 공유 키 9/8/7/1, 2:2 팀 턴제 경쟁, 스파이마스터-요원 역할 시야 분리, 암살자 즉시 패배)를 `/codenames/`로 추가. 기존 codenames-duet(2인 협력)과 독립 병존. 휴먼 전용(봇 미구현, 슬롯 구조 예약). smoke 65 + E2E 12 = 77 PASS, duet 회귀 27/27 무영향, QA PASS, AD3 APPROVED. 상세: `codenames/docs/`.
