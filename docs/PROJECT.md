...
> 최종 업데이트: 2026-07-28 — **별빛 우편탑 레디 화면 재설계 (UI 2단계).** 17개 레벨 카드를 4개 월드 탭(기지/자연/우주/경이)으로 분산, 카드 높이 압축(155px->99.6px), 준비 버튼 sticky footer 고정. 5개 해상도 전부 maxScroll=0, 겹침 0px, 준비 버튼 뷰포트 내 노출 달성. 1단계 CARRY-1(1024x576 스크롤 874px) 완전 해소. QA PASS, AD3 APPROVED.
...
| 루미큐브 | 2~4인 타일 게임 | 완료 |
| 오목 | 2인 오목 대전 | 완료 |
| 별빛 우편탑 | 협동 부스트와 장치 연동 복귀 경로를 사용하는 17개 레벨 LAN 2인 등반. CSS Grid 3행(topbar/play-viewport/bottombar) 레이아웃으로 메타 UI와 플레이 캔버스를 완전 분리. 레디 화면은 4개 월드 탭(기지/자연/우주/경이)으로 레벨을 분산 표시하고 sticky footer로 준비 버튼을 항상 노출. 플레이어별 안전 리스폰, AI·친구 2인 즉시 시작, 15초 재접속과 오래된 토큰 차단 지원 | 완료 |
