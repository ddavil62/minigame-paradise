...
> 최종 업데이트: 2026-08-01 — **테트리스 배틀 단일 글로벌 룸 → 멀티룸 아키텍처 전면 재설계.** 단일 `let players = []` 전역 상태를 `Map<roomId, Room>` 기반 다중 독립 2인 룸 구조로 교체. 10명 동시 접속 시 5개의 독립된 방이 자동 생성되어 각자 게임(가비지/아이템/게임오버/REMATCH) 상태를 격리 보유. `room` 파라미터 명시 입장(초대 링크), 파라미터 없으면 대기 중인 룸 자동 합류/신규 생성, AI 모드는 항상 전용 룸(`_botSpawnPending` 가드로 200ms 레이스 차단). 런처 로비 REDIRECT에 `roomId` 추가해 매칭된 2인이 동일 룸으로 진입. 회귀 17개 슈트 436건 + QA 독립 재검증 19건 = 455건 PASS(baseline 예외 3건 제외), AD3 APPROVED(레이아웃 영향 없음 확인).
