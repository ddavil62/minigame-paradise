/**
 * @fileoverview Entry UI Unify Phase 2 QA - tetris-battle
 *
...
test.describe('tetris-battle - Entry UI Phase 2 QA', () => {
  test.describe('AC-1: 직접 URL 접근 시 #screen-waiting 먼저 표시', () => {
    ...
    test('screen-waiting 초기 상태 스크린샷', async ({ page }) => {
      await page.goto(BASE);
      await page.waitForSelector('#screen-waiting', { state: 'visible', timeout: 5000 });
      await page.screenshot({ path: 'tests/screenshots/tetris-screen-waiting-initial.png' });
    });
  });

  test.describe('AC-2: #name-gate-inline - 닉네임 없으면 표시, 입력 후 hidden', () => {
    ...
    test('닉네임 게이트 표시 중 스크린샷', async ({ page }) => {
      ...
      await page.goto(BASE);
      await page.waitForSelector('#name-gate-inline', { state: 'visible', timeout: 5000 });
      await page.screenshot({ path: 'tests/screenshots/tetris-name-gate-visible.png' });
    });
  });

  test.describe('AC-3: #btn-start-ai 대기 화면에서 존재', () => {
  ...
  test.describe('AC-4: sessionStorage tetris-battle:name 저장', () => {
  ...
  test.describe('AC-5: VS 라벨 + 카운트다운 게임 화면 .center-area 유지', () => {
    test('center-area에 VS 라벨이 존재한다', async ({ page }) => {
      await page.goto(BASE);
      await page.waitForSelector('#screen-waiting', { state: 'visible', timeout: 5000 });
      const vsLabel = page.locator('.center-area .vs-label');
      await expect(vsLabel).toBeAttached();
      const text = await vsLabel.textContent();
      expect(text).toContain('VS');
    });

    test('center-area에 countdown이 존재한다', async ({ page }) => {
      await page.goto(BASE);
      await page.waitForSelector('#screen-waiting', { state: 'visible', timeout: 5000 });
      const countdown = page.locator('.center-area #countdown');
  ...
  test.describe('AC-7: 빈/공백 닉네임 제출 차단', () => {
    test('빈 닉네임 입력 시 제출이 되지 않는다', async ({ page }) => {
      await page.goto(BASE);
      await page.evaluate(() => sessionStorage.removeItem('tetris-battle:name'));
      await page.goto(BASE);
      await page.waitForSelector('#name-gate-inline', { state: 'visible', timeout: 5000 });
  ...
  test.describe('예외 시나리오', () => {
    ...
    test('sessionStorage에 이름이 있으면 게이트 없이 즉시 JOIN', async ({ page }) => {
      await page.goto(BASE);
      await page.evaluate(() => sessionStorage.setItem('tetris-battle:name', 'ReturningUser'));
      await page.goto(BASE);
      await page.waitForSelector('#screen-waiting', { state: 'visible', timeout: 5000 });
      const gate = page.locator('#name-gate-inline');
