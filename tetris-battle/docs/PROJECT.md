...
> 최종 업데이트: 2026-08-03 — 런처 경유 매칭 결함 수정. 런처가 발급한 roomId를 게임 서버가 인식하지 못해 런처 경유 매칭(일반 2인 + AI 채우기)이 항상 "Room not found"로 실패하던 결함을 `ensureRoom(id)` 선제 등록으로 해소하고, `GAME_MANAGED_AI_IDS`에 편입해 게임 자체 봇 spawn 로직을 재사용하도록 정리. 신규 13단언 포함 회귀 174건 PASS.
