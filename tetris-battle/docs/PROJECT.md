...
> 최종 업데이트: 2026-07-27 — 아이템 지급을 고유 라인 클리어 이벤트별 서버 권위 80% 확률로 복원했다. 390×844 세로 화면에서 보드·ITEMS·NEXT·상대 미니맵을 모두 표시하며 리포트 32 QA 123/123 PASS.
...
| 모듈 | 파일 | 역할 |
|---|---|---|
| 서버 | `server.js` | 2인 1룸 중계, 고유 클리어 이벤트별 80% 확률·아이템 슬롯 권위 처리, 전체 보드 검증·중계, LAN IP 감지, `mode=ai` 봇 관리 |
