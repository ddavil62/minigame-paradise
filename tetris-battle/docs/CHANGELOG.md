...
## [2026-08-13] 공격 아이템 비행 연출

- 공격 아이템 사용 시 모든 참가자 화면에서 공격자 보드 중앙부터 타깃 보드 중앙까지 반투명 아이콘이 직선으로 비행한다.
- 이동 시간은 1000ms이며 빠르게 출발해 타깃 근처에서 감속하는 이징을 사용한다. `공격자 번호 → 타깃 번호`, 약한 직선 궤적, 출발 섬광, 도착 확대·섬광·흔들림을 함께 표시한다.
- 방어막 차단 시에는 타깃 방어막 앞에서 아이콘이 확대·회전하며 파편처럼 사라지는 별도 충돌 연출을 사용한다.
- 공격형 3종에만 적용하고 일반 라인 가비지와 자기 대상 아이템은 제외한다. 동시 비행은 최대 4개로 제한하며 저감 모션에서는 160ms 출발·도착 강조로 대체한다.
- 서버가 `ITEM_ATTACK`에 공격자·타깃·아이템·차단 여부를 담아 전원에게 전송하며 6인 통합 테스트에서 동일 이벤트 수신을 검증한다.

## [2026-08-13] 상대 보유 아이템 공개

- 경기 중 각 상대 보드 아래에 서버 권위 아이템 슬롯 3개를 슬롯 순서 그대로 표시한다. 획득·사용 즉시 전원 화면에 동기화하며 빈 슬롯은 낮은 명도로 표시한다.
- 2인전은 내 아이템 패널과 비슷한 크기로, 3인 이상은 미니보드 폭에 맞춘 아이콘형 슬롯으로 표시한다. 탈락자의 슬롯은 비활성 상태로 남긴다.
- 보유 중인 방어막은 슬롯에, 이미 사용해 활성화된 방어막은 기존 보드 배지에 표시해 두 상태를 구분한다.
- 6인 WebSocket 테스트에서 획득·사용 후 슬롯 상태 동기화를 검증하고, 2인·6인 Chromium 테스트에서 상대별 슬롯 개수와 레이아웃을 확인했다.

## [2026-08-13] 2인전 동일 크기 상대 보드

- 2인전에서는 상대 보드를 내 보드와 정확히 같은 가로·세로 크기로 표시한다.
- 3인 이상에서는 기존 최대 5개 상대 보드의 3×2 미니보드 배치를 유지한다.
- 참가 인원에 따라 레이아웃 클래스를 자동 전환하며 데스크톱·모바일 보드 높이 제한을 각각 적용한다.
- Chromium 시각 테스트에서 2인 보드 크기 일치와 6인 3×2 배열 유지를 함께 검증했다.

## [2026-08-13] 2~6인 테트리스 배틀

- 방 정원을 6명으로 확장하고 현재 입장 인원 전원이 READY일 때 2~6인으로 경기를 시작한다. 번호는 입장 순서대로 고정되며 대기실부터 번호·닉네임·준비 상태를 표시한다.
- 상대 보드를 최대 5개까지 3×2 그리드로 렌더링한다. 탈락자는 보드를 유지한 채 관전 상태가 되고 입력과 아이템 사용이 차단되며 마지막 생존자가 승리한다.
- 3인 이상 공격 아이템은 600ms 내 `아이템 슬롯 번호 → 대상 플레이어 번호` 조합으로 사용한다. 자기 대상 아이템과 2인전 공격 아이템은 슬롯 숫자 한 번으로 사용한다.
- 일반 가비지는 사용자의 최근 공격 아이템 대상이 생존 중이면 그 대상에게, 없으면 무작위 생존 상대에게 전송한다. 잘못되거나 탈락한 대상은 서버가 거절하고 아이템을 복원한다.
- 6인 WebSocket 통합 테스트와 Chromium 6페이지 시각 테스트로 준비 잠금, 타기팅, 탈락, 최후 생존 승리 및 3×2 배치를 검증했다.
