...
> 공개 HTTPS 통합 런처의 1:1 끝말잇기 대전 (가비지 음절 공격). Node.js + 바닐라 JS. 미니게임 천국 16번째 종목 (개발 포트 3008). 친구는 같은 공유기에서도 `https://112.155.2.238`로 접속하며 개별 포트 직접 접속은 안내하지 않는다.
...
|---|---|
| 플레이어 | 2인 (LAN 1:1) |
| 체인 | 하나의 공유 체인을 두 플레이어가 번갈아 이어감 |
| 단어 검증 | 서버 권위. 로컬 한국어 사전 (12만+ 단어) |
| 두음법칙 | 허용 (DUEUM_MAP 테이블) |
| HP | 100 시작. 0이면 패배 |
| 게이지 | 제출 시 남은 시간 × 5% 충전 (10초=50%, 1초=5%) |
| 가비지 공격 | 게이지 100 도달 시 자동 발동. 상대에게 강제 시작 글자 + 10 HP 데미지 |
| 첫 시작 글자 | 매치마다 유효 후보 중 무작위 지정 |
| 타이머 | 턴마다 10초. 만료 시 해당 플레이어만 5 HP 페널티 후 상대에게 턴 전달 |
| 리매치 | 양쪽 동의 시 재시작 |
| 상대 입력 표시 | 상대 턴에는 현재 입력 중인 글자 수만 실시간 표시 |
| AI 봇 | 서버 관리형 보통 AI 지원 |
