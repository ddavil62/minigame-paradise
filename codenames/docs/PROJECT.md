...
> 최종 업데이트: 2026-06-28 — 신규 프로젝트(1차 코어 완료)

## 개요

LAN 2:2 정통 코드네임. 미니게임 천국 **11번째** 종목. 기존 codenames-duet(2인 협력)과 독립 병존하며, 정통 룰(단일 공유 키, 2팀 턴제 경쟁, 역할별 시야 분리)을 구현했다. 이번은 **휴먼 전용 4인**(AI 봇 미구현, 슬롯 구조만 예약).
...
- ✅ WS 서버 (`server.js`) — 3단계 phase, role_select 대기실, createApp factory, 단독 실행 + 포트 폴백, heartbeat
- ✅ 클라이언트 UI (`public/`) — role_select 선택 화면 + 게임 보드 + 단서/추측 패널 + 결과 모달
- ✅ launcher 통합 — `games.json` 11번째 카드(id `codenames`, port 3014, botAvailable=false) + `launcher/server.js` GAME_APPS 등록(URL 첫 세그먼트로 codenames-duet과 정확 분리)
- ✅ 룰북 `docs/RULEBOOK.md` (§1~§13)
- ✅ 테스트 — smoke 65 + E2E 12 = 77 PASS, codenames-duet 회귀 27/27 무영향, 역할 마스킹 시각 검증(요원 키 누설 0)
- ✅ QA PASS(blocker 0) / AD3 APPROVED(REVISE 6건 반영)

## 알려진 한계

- **AI 봇 미지원** — 휴먼 전용 4인. `Player.ws` 슬롯 + `createApp(getBotUrl)` 훅을 예약했으나 봇 실구현은 추후 발주(RULEBOOK §13-2).
...
- AI 봇(`bot.js`) — 단서 생성 휴리스틱 설계 + `getBotUrl` 훅 활성화
