...
공개 HTTPS 통합 런처의 2:2 정통 코드네임. 친구는 같은 공유기에서도 `https://112.155.2.238`로 접속하며 개별 포트 직접 접속은 운영하지 않는다. 미니게임 천국 **11번째** 종목. 기존 codenames-duet(2인 협력)과 독립 병존하며, 정통 룰(단일 공유 키, 2팀 턴제 경쟁, 역할별 시야 분리)을 구현했다. **AI 봇 지원**(완전 오프라인 태그맵 기반) — 사람 1~3명 + 봇으로 시작 가능.
